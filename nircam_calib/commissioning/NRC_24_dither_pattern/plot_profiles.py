...
#
#
def plot_profiles(sca, sim, filter, type, observation,car):
    print("sca is ", sca)
    print("sim is ", sim)
    ...
    if(host == 'ema.as.arizona.edu'):
        plot_dir = "/home/cnaw/Pictures/car_"+car+"/"
        if(sim == "mirage"):
            target_dir = '/home/cnaw/commissioning/car_24_apt_01073/mirage/analysis/'
            string   = target_dir+'*'+observation+'001*_*'+sca+'_'+type+'_'+filter+'*prof.fits'
        else:
            target_dir = '/home/cnaw/commissioning/car_24_apt_01073/guitarra/analysis/'
            if(type == "slp"):
                string   = target_dir+'*'+observation+filter+'*'+sca_number[sca]+'*'+type+'*prof.fits'
            else:
                string   = target_dir+'*'+observation+'001*'+sca+'_'+filter+'*_'+type+'_*prof.fits'
    ...
    if(host == 'orange.as.arizona.edu'):
        ...
        if(sim == "mirage"):
            ...
            if(type == "slp"):
                string   = target_dir+'apt*'+observation+filter+'*'+sca_number[sca]+'*'+type+'*prof.fits'
            else:
                string   = target_dir+'*'+observation+'001*_*'+sca+'_'+type+'_'+filter+'*prof.fits'
...
#    type        = "cal"
    ...
    plot_profiles(args.sca, args.sim, args.filter, args.type, args.obs, args.car)
