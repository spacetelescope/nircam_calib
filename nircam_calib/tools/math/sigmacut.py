...
class calcaverageclass:
    ...
    def calcaverage_sigmacut(self,data,mask=None,mean=None,stdev=None,Nsigma=None,fixmean=None,medianflag=False,percentclip=None,verbose=0):
        if mean!=None:
            self.mean=mean
        if stdev!=None:
            self.stdev=stdev
        ...
        if percentclip!=None:
            N = data4use.size
            Nclip = int(percentclip/100.0*N)
            if Nclip>1:
                tmp = scipy.sort(data4use, axis=None) 
                lowval = tmp[Nclip]
                highval = tmp[-Nclip]
                if verbose:
        ...
        if self.Nused>0 and (not(self.stdev is None)):
            self.mean_err = self.stdev/scipy.sqrt(self.Nused)
            self.stdev_err = self.stdev/scipy.sqrt(2.0*self.Nused)
        else:
            self.mean_err = None
            self.mean = None
            self.stdev_err  = None
    ...
    def calcaverage_sigmacutloop(self,data,mask=None,noise=None,Nsigma=3.0,Nitmax=10,fixmean=None,verbose=0,saveused=False,median_firstiteration=True,percentclip_firstiteration=None):
        """
        ...
        while ((self.i<Nitmax) or (Nitmax==0)) and (not self.converged):
            ...
            if percentclip_firstiteration!=None and  (self.i==0):
                percentclip=percentclip_firstiteration
            else:
                percentclip=None
            if noise is None:
                self.calcaverage_sigmacut(data,mask=mask,Nsigma=Nsigma,fixmean=fixmean,medianflag=medianflag,percentclip=percentclip,verbose=verbose)
            else:
